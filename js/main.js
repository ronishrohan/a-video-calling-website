...
if(name ==null || !name || name== undefined || name === 'null'){
  let tempName = prompt("Enter your display name");
  name = tempName;
...
var joinNotif = new Audio("../assets/notification_ambient.wav");
...
async function subscribe(user, mediaType, userID) {
  ...
  if (!participants.includes(inName)) {
    ...
    document
      ...
      .insertAdjacentHTML("beforeend", participant);
  }
...
function handleUserUnpublished(user, mediatype) {
  if (mediatype === "video") {
    ...
    sendBotMessage(`${id} left the room`);
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  sendBotMessage("welcome to the room ฅ^•ﻌ•^ฅ");
});
...
function sendBotMessage(message) {
  ...
  document
    ...
    .insertAdjacentHTML("beforeend", botMessage);
  let targetElement = Array.from(
    document.querySelectorAll(".npc-message")
  ).pop();
